...
Download a sample data file: [worms.csv](intro/uploadFiles/worms.csv). (You can view this file in a text editor if you so wish.) We can load this into R using the `read.csv()` function, here storing the resulting `data.frame` as an R object called `worms`:
...
```{r, echo = FALSE}
worms <- read.csv("intro/uploadFiles/worms.csv")
```
