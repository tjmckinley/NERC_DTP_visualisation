...
All required data files can be downloaded from ELE`r ifelse(!is_latex_output(), " or [here](advis/uploadFiles/datasets_advis.zip)", "")`.
```
...
```{info, title = "Note", collapsible = FALSE}
Great resources are the [Data Import Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/data-import.pdf), [Data Tidying Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/tidyr.pdf) and [Data Transformation Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/data-transformation.pdf), which give lots of information about functions available in different `tidyverse` packages[^tidyversepackages].
```
...
Notice once again that the first argument to `arrange()` is the data set, and then subsequent arguments are the columns that we wish to order by. A `desc()` function wrapped around a variable name means that the sort is in *descending* order, e.g.
...
```{r, echo = FALSE}
head(arrange(iris, Species, desc(Sepal.Length)))
```

Again, we do not require the `$` operator here.
...
See the [Data Transformation Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/main/data-transformation.pdf) for more examples.
...
iris %>% 
    group_by(Species) %>%
    summarise(mn = mean(Sepal.Length), .groups = "drop")
```

The `summarise()` function (note, this is different to the `summary()` function), applies a function to a `data.frame` or subsets of a `data.frame` (the `.groups = "drop"` argument just drops the grouping information after the summaries have been completed---there are other options if required). 
...
iris %>% 
    ...
    summarise(mnL = mean(Sepal.Length), varL = var(Sepal.Length),
        mnW = mean(Sepal.Width), varW = var(Sepal.Width), .groups = "drop")
``
...
Note that there is also a useful shortcut function: `across()`, that enables you to apply a transformation or summary to multiple columns without having to type everything out long-hand. For example, you can use `across()` within `summarise()` functions to apply the same function to multiple columns (rather than having to replicate the code for each column separately). For example, to generate the mean for all numerical columns, grouped by `Species`, you could use:
...
Here the `where(is.numeric)` is a selection helper function that returns columns where `is.numeric()` is `TRUE`, and `across()` then operates across these columns to apply the function `mean()`.

You can also concatenate column numbers, or explicit column names, or some of the `select()` helper functions within `across()`. For example, to produce the mean sepal length and width, grouped by `Species`, then you could use either

```{r}
iris %>% 
    group_by(Species) %>%
    summarise(across(c(1, 2), mean), .groups = "drop")
```
...
iris %>% 
    group_by(Species) %>%
    summarise(across(c(Sepal.Length, Sepal.Width), mean), .groups = "drop")
```
...
To apply multiple functions to each column is a bit trickier. In this case you need to provide a list of functions to apply. For example, to generate the mean and standard deviations for sepal widths and lengths, grouped by `Species`, you could use:

```{r}
iris %>% 
    group_by(Species) %>%
    summarise(across(starts_with("Sepal"), list(Mean = mean, SD = sd)), .groups = "drop")
```

Note that since you are applying multiple functions, it's more informative to use a named list e.g. `list(Mean = mean, SD = sd))`, where `Mean` and `SD` here correspond to how you want the output columns labelled for each function (`mean()` and `sd()` respectively). The output names are appended to the column names in the resulting `tibble` object. See [`vignette("colwise")`](https://dplyr.tidyverse.org/articles/colwise.html) for more details.

```{info, title = "Passing additional arguments to functions", collapsible = TRUE}
Note that if you want to pass additional arguments to a function used in `across()`, then you can utilise a special syntax using `~` as shown below. For example, imagine that we had a missing value (`NA`) in one entry of `Sepal.length`, then running `mean()` will return an `NA` unless we tell R to remove the `NA` values explicitly (using the `na.rm = TRUE` argument to `mean()`) i.e.

``{r}
iris1 <- iris
iris1$Sepal.Length[1] <- NA
iris1 %>%
    group_by(Species) %>%
    summarise(across(starts_with("Sepal"), mean), .groups = "drop")
``

Notice the `NA` in the summary for the `setosa` group caused by the missing data point. Compare this to the version below where we use `~mean(.x, na.rm = TRUE)`, which is a shorthand for `function(.x) mean(.x, na.rm = TRUE)`. (Essentially each column in `across()` gets passed as the `.x` variable into the function.) The use of `.x` as the variable name is a consequence of using the `~` notation. You could also explicitly write your own function and use whatever local variable name you like. See various options below for examples.

``{r}
iris1 %>% 
    group_by(Species) %>%
    summarise(across(starts_with("Sepal"), ~mean(.x, na.rm = TRUE)), .groups = "drop")
``
...
iris1 %>% 
    group_by(Species) %>%
    summarise(
        across(starts_with("Sepal"), function(vec) mean(vec, na.rm = TRUE)), 
        .groups = "drop"
    )
``
...
mean_rmNA <- function(vec) {
    mean(vec, na.rm = TRUE)
}

iris1 %>% 
    group_by(Species) %>%
    summarise(across(starts_with("Sepal"), mean_rmNA), .groups = "drop")
``

```

Note that you can also use `across()` within `mutate()` calls as well, for example, if you had a data frame called `DATASET` then you could use e.g.

```{r, eval = FALSE}
mutate(DATASET, across(where(is.character), as.factor)) %>%
    summary()
```

as a quick way to create an informative summary, where any `character` column is temporarily converted to a `factor` for the purposes of the summary output (so `summary()` returns the number of observations for each level of the corresponding factor, instead of simply telling you that a column is a `character`). This leaves all non-numerical columns alone.
...
Let's pull up some data. You should have access to a `.csv` file called `r ifelse(!is_latex_output(), "[indicator gapminder gdp_per_capita_ppp.csv](advis/uploadFiles/indicator gapminder gdp_per_capita_ppp.csv)", "indicator gapminder gdp_per_capita_ppp.csv")`. This is a digital download of the [Gapminder](https://www.gapminder.org/) GDP per capita data that can be found in the `gapminder` package. All the data sets used in the [Gapminder](https://www.gapminder.org/) project can be downloaded from [https://www.gapminder.org/data/](https://www.gapminder.org/data/).
...
The next thing we need to do is to collapse the **year** columns down. Ideally we want a column corresponding to **country**, a column corresponding to **year** and a final column corresponding to **GDP**. We are going to do this by using the `pivot_longer()` function. Note the key arguments to `pivot_longer()` are:

* `data`: this gives the name of the data frame;
* `cols`: the columns to pivot into longer format;
* `names_to`: the name of the column to create from the data currently stored as column names (e.g. `1800`, `1801` etc.);
* `values_to`: gives the name of the columns that will contain the **values** in each of the cells of the collapsed column (e.g. the corresponding GDP values);

Here we want to collapse everything ***except*** `country`, which we can do using the `!` operator.
...
gp_income %>% 
    group_by(country) %>%
    summarise(mn = mean(gdp), .groups = "drop")
```
...
gp_hiv <- read_csv("indicator hiv estimated prevalence 15-49.csv") %>%
            ...
            pivot_longer(!country, names_to = "year", values_to = "prevalence") %>%
            mutate(year = as.numeric(year)) %>%
            filter(!is.na(country)) %>%
...
```{info, title = "Note", collapsible = FALSE}
In case you haven't managed to complete the last task, the `gp_hiv` data is available as `r ifelse(!is_latex_output(), "[gp_hiv.rds](advis/uploadFiles/gp_hiv.rds)", "gp_hiv.rds")`. You can read this into R using `gp_hiv <- readRDS("gp_hiv.rds")` and continue with the practical.
```
...
gp_pop <- read_csv("indicator gapminder population.csv") %>%
                pivot_longer(!`Total population`, names_to = "year", values_to = "pop") %>%
...
Here we look at the population counts for three countries: Germany, Mexico and the US from the year 2000. You should have access to three files: `r ifelse(!is_latex_output(), "[Germanypop.csv](advis/uploadFiles/Germanypop.csv)", "Germanypop.csv")`, `r ifelse(!is_latex_output(), "[Mexicopop.csv](advis/uploadFiles/Mexicopop.csv)", "Mexicopop.csv")` and `r ifelse(!is_latex_output(), "[USpop.csv](advis/uploadFiles/USpop.csv)", "USpop.csv")`, each with the following columns:
...
pop <- germany %>%
        ...
        pivot_longer(!age, names_to = "country", values_to = "pop") %>%
...
pop %>%
    ...
    select(!pop) %>%
    pivot_wider(names_from = age, values_from = cumprop)
```

```{task}
Go through the workflow above and understand what each line is doing and why. Add comments to the code. If you don't understand, please ask one of the demonstrators. Notice that we have used the `pivot_wider()` function to produce a table that is **not** in 'tidy' format, but is better for visualising these specific outcomes. Note also the `.groups = "drop_last"` argument to summarise, which ensures that the `age` grouping is dropped, but not the `country` grouping, which is needed to correctly calculate the cumulative proportions *within each country*.
```
...
```{r, screenshot.alt = "advis/images/population.jpg", echo = FALSE, eval = opts_knit$get("rmarkdown.pandoc.to") == "html", out.width = "100%"}
include_url("https://www.youtube.com/embed/QsBT5EQt348")
```

```{r, eval = opts_knit$get("rmarkdown.pandoc.to") == "latex", echo = FALSE, results = "asis"}
cat("\\begin{center}\\href{https://www.youtube.com/embed/QsBT5EQt348}{\\includegraphics[width = 0.8\\textwidth]{advis/images/population.jpg}}\\end{center}")
```
