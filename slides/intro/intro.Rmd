...
There will also be some formative worksheets to help you consolidate these ideas as we go through the module. Full answer sheets will be provided.

## Summative assessments

There will be one piece of coursework, in the form of a programming report. This will be worth 40% of the total module mark.

There will also be a 2.5hr (open-book) practical exam worth 60% of the module mark (**Option 4 exam**).
...
(Here this calculates $\sqrt{e^y} = \sqrt{e^{(2 + 2)}}$.) Also, remember e.g. BODMAS:
...
A quick way to save your **workspace** is to use `save.image("NAME_OF_FILE.RData")`.
...
`.RData` files can be read in using `load("NAME_OF_FILE.RData")`
...
This has the advantage that the file can be easily read back in using e.g.
