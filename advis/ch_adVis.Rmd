...
```{info, title = "Note", collapsible = FALSE}
There is also a `facet_grid()` option that allows us to facet by more than one variable. Please see the [Data Visualisation Cheat Sheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) for more details.
```
...
```{r, eval = ifelse(is_latex_output(), TRUE, FALSE)}
