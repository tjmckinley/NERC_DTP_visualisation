...
gp_income <- read_csv("advis/indicator gapminder gdp_per_capita_ppp.csv")
gp_income
```
...
``{r, message = FALSE, eval = FALSE}
gp_hiv <- read_csv("advis/indicator hiv estimated prevalence% 15-49.csv") %>%
...
> In case you haven't managed to complete the last task, the `gp_hiv` data is available as "gp_hiv.rds". You can read this into R using `gp_hiv <- readRDS("advis/gp_hiv.rds")` and continue with the practical.
...
``{r, message = FALSE, eval = FALSE}
## read in data
gp_pop <- read_csv("advis/indicator gapminder population.csv") %>%
...
```{r, message = FALSE, eval = FALSE}
## read German data in
germany <- read_csv("advis/Germanypop.csv")

## read Mexican data in
mexico <- read_csv("advis/Mexicopop.csv")

## read US data in
us <- read_csv("advis/USpop.csv")
```
...
```{r, screenshot.alt = "advis/images/population.jpg", echo = FALSE}
