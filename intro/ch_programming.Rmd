...
A PDF handout for this part of the module can be found on the ELE page or via the link [here](intro/uploadFiles/progHANDOUT.pdf). A PDF version of the slides (not in handout form) can be found [here](intro/uploadFiles/progSLIDES.pdf), and a HTML version (which should be compatible with screen-readers) can be found [here](intro/uploadFiles/progSLIDES.html).
...
```{info, title = "Note", collapsible = FALSE}
A downside of `stopifnot()` is that the error messages are not particularly informative. A more informative, but more verbose function is called `stop()`, and this allows you to output custom error messages. For example, you could use an explicit `if()` statement alongside `stop()` to produce a more informative error message:
\newline
``{r, eval = FALSE}
## check `x` variable is present
if(missing(x)) {
    stop("'x' argument is missing from function")
}
``
```
...
Their sole *raison d'&ecirc;tre* (as far as I can tell) is to make code more concise. Essentially they allow you to replace the use of a loop with a single line of code that aids legibility. They do not (as far as I'm aware) make the code faster (for that you can see various functions in packages like `dplyr`.
