...
A PDF handout for this part of the module can be found on the ELE page or via the link [here](intro/uploadFiles/IntroToRHANDOUT.pdf). A PDF version of the slides (not in handout form) can be found [here](intro/uploadFiles/IntroToRSLIDES.pdf), and a HTML version (which should be compatible with screen-readers) can be found [here](intro/uploadFiles/IntroToRSLIDES.html).

All data files needed can be downloaded from ELE, or via the link [here](intro/uploadFiles/datasetsIntro.zip).
```
...
If you want to download R for your own computer, first go to [http://cran.r-project.org](http://cran.r-project.org), and follow the instructions in the box called 'Download and Install R'. Once you have installed R, you can install RStudio from [https://www.rstudio.com/](https://www.rstudio.com/). University machines should have these pieces of software already installed, so just load RStudio and you're good-to-go!
...
Notice that the `#` symbol in the input code denotes a **comment**, such that any text after the `#` is ignored (up to the end of the current line). I have used a double hash here `##`, though this is simply because I've written this practical using [RMarkdown](http://rmarkdown.rstudio.com/) (more on this later), and it seems to typeset better. This is not absolutely necessary though, since anything after the first `#` is ignored. 
...
For example, I once had an issue with a package called [`cowplot`](https://cran.r-project.org/web/packages/cowplot/), that didn't exist for my version of R, and returned the following error message:
...
I noted the version number, and then used `install_version()` from the `devtools` package as follows:
