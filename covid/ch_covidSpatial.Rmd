...
```{info, title = "Note", collapsible = FALSE}
In some versions of `tidyverse` in seems that the `date` column will not be automatically converted to a `Date` object when the data is read in (or if you have used `read.csv()` instead of `read_csv()`). As such the code below will sort into the wrong order (because `date` will be a `character` vector and not a `Date` vector). If you get this behaviour, then you will have to manually convert `date` from a `character` to a `Date` using `as.Date()`. For example:
    
``{r, eval = FALSE}
covid <- mutate(covid, date = as.Date(date, format = "%Y-%m-%d"))
``

Remember, the `format` argument to `as.Date()` defines the format that the dates appear in the vector that you want to convert. Here they are specified as a four-digit year (`%Y`), followed by a two-digit month (`%m`), followed by a two-digit day (`%d`), each separated by hyphens (`-`). See [earlier](#dateinfo) for more details.
```

Assuming the `date` column is in the correct format, then to calculate the new deaths at each day within each LTLA we can run:
...
``{r, fig.width = 10, fig.height = 5, out.width = "80%", warning = FALSE}
## produce time-series plot of deaths
ggplot(largestDeaths) +
    geom_line(aes(x = date, y = deaths, colour = areaName)) +
    xlab("Date") + ylab("Deaths within 28 days of a positive test") +
    labs(colour = "LTLA")
``
...
```{r, echo = FALSE, fig.width = 10, fig.height = 5, out.width = "80%", message = FALSE, warning = FALSE}
## produce time-series plot of deaths
...
```{info, title = "Note", collapsible = FALSE}
Some students reported getting an error when trying to run the code below, of the form:

``
Error: arguments have different crs
``

This seems to have been caused by a bug in the latest version (`0.1.4`) of the `transformr` package that `gganimate` uses. If you get this error, then a quick solution is to roll-back to the previous version of `transformr`. To do this, you can use the `remotes` package as follows (if `remotes` is not installed, then it can be installed in the usual way using `install.packages("remotes")`):

``
library(remotes)
install_version("transformr", version = "0.1.3")
``

Then you will have to restart R and rerun your code but then the animation should work. Let me know if you have any further problems.
```
