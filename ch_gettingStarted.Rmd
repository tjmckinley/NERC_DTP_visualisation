...
The RStudio window should look something like Figure \@ref(fig:rstudio).
...
* **Console pane** (bottom-left): This is where you run R commands and view outputs. Interfaces directly with R.
...
The helpful folk at RStudio also produce a series of excellent Cheat Sheets, available [here](https://www.rstudio.com/resources/cheatsheets/). Please note, these are updated semi-regularly as new packages are added or existing packages updated. Note also that these cheat sheets focus on the use of RStudio, and a small number of subset of packages that are developed by RStudio (e.g. `tidyr`, `dplyr`, `ggplot2`, `shiny` and R Markdown). For example, a nice Cheat Sheet for RStudio itself can be found [here](https://www.rstudio.com/wp-content/uploads/2016/01/rstudio-IDE-cheatsheet.pdf).

I will provide links to some of these cheat sheets as we progress through the practicals, but please note that they might change over time, and older versions exist online.
...
You can see here that R has returned a value of `r 10 + 5 * 3`, illustrating one of R's key features: that it can be used as an overgrown calculator, simply by entering commands into the prompt. Notice that (unlike C) you did not have to compile these commands, you simply type them into the console, hit return, and R produces an answer. R supports basic mathematical operators, such as those found in Table \@ref(tab:mathop).
...
One important thing to note is that unlike C, R does not require the semicolon (`;`) symbol to denote the end of each command. A carriage return is sufficient. A semicolon can be used to allow multiple commands to be written on the same line if required. For example, 
...
The console window is the engine room of R, and one can interact directly with it. It is possible to save the command history, or run back through it using the arrow keys. However, a much better approach is to use the **script pane** to write an R script that contains all the commands necessary for a particular project. In fact, one might argue that this is probably one of the most important features of R relative to a point-and-click statistical package such as SPSS.
...
Notice that nothing has happened. All you've done is write some commands into a text window. However, if you highlight these lines and then hit the `Run` button in the top right-hand corner of the script pane (or, in Windows certainly, press `Ctrl-R`), then RStudio runs these lines in the console pane. (Alternatively, you can manually copy-and-paste these lines into the console window.) This should return:
...
It is conventional to save R script files using the suffix '.R' or '.r', though remember that they are simply **text files**, and can be viewed in any text editor. Make sure you have set up a folder to store the code for this practical in, and have changed the **working directory** to this folder (see the [setup](#setup) section), and then save this script file as something like "IntroToR.R".

> Make sure you save your script file regularly to prevent data loss!

#### Notes on legibility {#legible}

> **NOTE**: that I use spaces within the code to make it clearer. R does not require this, but again I think it is good practice to think about how to make you code legible. Different coders have different preferences, but personally I prefer `plot(Worm.density ~ Vegetation, data = worms)` over `plot(Worm.density~Vegetation,data=worms)`. 

> For interest, some guidelines from Google can be found at [https://google.github.io/styleguide/Rguide.xml](https://google.github.io/styleguide/Rguide.xml). Note that different coders prefer different styles---there is no universal agreement. (For example, I have my own idiosyncratic style.) However, it's worth getting into the habit of writing your code neatly and with a thought towards legibility...

> I tend to use script files to keep a record of all commands that need to be reproduced, but I often enter commands directly into the console window when I'm testing or visualising things. Try to keep redundant code out of your script files. In this practical I expect you to use script files from the outset! Make sure the code is legible and commented.

An example of a tidy R script is given e.g. [here](#tidyscript).
...
> **Note**: R packages can be thought of in a similar way to Matlab toolboxes.
...
> This will ask that you select a repository---choosing one close-to-home is a good idea, I usually go for Cambridge or Bristol. It might also ask you to set up a local R library in your user directory. This is a good idea, so I would just accept the default if it asks.
...
For example, I recently had an issue with a package [`cowplot`](https://cran.r-project.org/web/packages/cowplot/) not existing for my version of R:
