...
There has recently been a package released called [`gt`](https://gt.rstudio.com/), which looks awesome. It is currently still in the early release stage (although it is on CRAN), and I haven't used it in anger yet, which is why I haven't expanded on it here. However, feel free to install and have a play if you like!
```
...
R Markdown also supports bibliographic referencing using a BibTeX-style bibliographic manager. This is a format used in LaTeX documents, but in essence simply means that rather than use a manager such as EndNote, Zotero or Mendeley, instead you record your references in a simple text file with the suffix `.bib`. The references must be written in a specific format, but these can easily be downloaded through the **citation download** button on most journal papers. For the format of different types of reference, such as articles, books or technical reports, please see [https://en.wikipedia.org/wiki/BibTeX](https://en.wikipedia.org/wiki/BibTeX). Note that you can usually convert bibliographies stored in other formats, e.g. Zotero, to BibTeX if required.

An example BibTeX entry for [Partridge and Farquhar (1981)](https://www.nature.com/articles/294580a0#citeas) is:
...
I wrote this by hand. Conversely, Figure \@ref(fig:citation) shows an example where the BibTeX entry can be downloaded directly, for [Spiegelhalter (2020)](https://www.bmj.com/content/370/bmj.m3259).
...
If we save all the references we need into a file called e.g. `references.bib` (stored in the working directory), then we can add this file to the YAML as follows:
...
The first part of the BibteX entry, `partridge_farquhar:1981` is a unique identifier for the reference, and can be changed to anything you like. This identifier can then be used in the code as e.g. `@partridge_farquhar:1981`, which will then typeset as Partridge and Farquhar (1981), and `[@partridge_farquhar:1981]` will typeset as (Partridge and Farquhar, 1981). This will then add the references to the end of the document. It's usually worthwhile adding an empty section to the end of your R Markdown code with the title **References** e.g.
...
```{info, title = "Note", collapsible = FALSE}
Only entries that are cited in the document will appear in the reference list, so you are free to use a single `.bib` file for all your references if you like, and pick-and-choose the ones you need as and when you need them.
```

```{info, title = "Note", collapsible = FALSE}
By default this uses a Chicago referencing style, which is an author-year style. Other formats (e.g. Vancouver, Harvard etc.) can be specified using Citation Style Language (CSL) files. A good resource for these is the [Zotero Style Guide](https://www.zotero.org/styles), and once downloaded they can be added to the YAML and then update the document accordingly.
...
```{task}
Add a reference to [Partridge and Farquhar (1981)](https://www.nature.com/articles/294580a0#citeas) to your `ff.Rmd` using the approach described above.
```
