...
Here we can see that the data set is large: `r format(nrow(covid), big.mark = ",")` observations across `r format(length(unique(covid$areaCode)), big.mark = ",")` LTLA regions. Not the sort of data set we want to manipulating with point-and-click software!
...
For ease, the version of the data used in the analysis below is found in the  `r ifelse(!is_latex_output(), "[Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC.zip](covid/uploadFiles/Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC.zip)", "*Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC.zip*")` file, which has been downloaded from:
...
```{r, eval = FALSE}
ltla <- st_read("Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC/Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC.shp")
ltla
```

```{r, echo = FALSE}
ltla <- st_read("covid/Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC/Local_Authority_Districts_(December_2019)_Boundaries_UK_BUC.shp")
ltla
```
...
for(i in 2:length(spatial_gif1_img)) {
    ...
    spatial_gif <- c(spatial_gif, image_composite(spatial_gif1_img[i], spatial_gif2_img[i], offset = "+200+100"))
}
