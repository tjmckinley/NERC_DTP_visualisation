...
## Expressing the population fatality risks of COVID in different ways

The table below is a replication of Table 2 in Spiegelhalter (2020), and expresses the age-specific population fatality risks of COVID-19 in different ways.
...
## add different columns denoting risk estimates
...
## tidy data and amend age classes
...
## read in the correct worksheet for weekly deaths
...
## join with table 2 and add remaining columns
...
```{r}
## extract columns for table and rearrange to get correct age-classes
## in correct place (there are various ways to do this)
select(table2, age, deaths, population, PFR, percentage, PFR1, nonCovidDeaths, weeklyDeaths, perc5yearRisk, addDaysRisk) %>%
    ...
    mutate(PFR1 = paste0("1 in ", format(PFR1, digits = 0, big.mark = ",", scientific = FALSE))) %>%
    ...
    mutate(perc5yearRisk = format(perc5yearRisk, digits = 0)) %>%
    mutate(addDaysRisk = format(addDaysRisk, digits = 0)) %>%
    kable(
        ...
        align = "c",
        caption = "Mortality rates expressed in different ways",
