...
* Columns that can't be easily converted are stored as `character` columns (e.g. the `Vegetation` column). You can tell this because the `summary()` function returns no useful summaries for these columns except to say they are `character` columns[^charcols].
