...
The full paper---Spiegelhalter D. Use of "normal" risk to improve understanding of dangers of covid-19, *BMJ*, 2020; 370 :m3259---can be found [here](https://www.bmj.com/content/370/bmj.m3259).

Interestingly, this paper used little statistical modelling. Rather, it used publicly available data and insightful data visualisation to make its main points. Here we will focus on reproducing Figure 1 from this paper, which is replicated in Figure @ref(fig:spiegelhalter).
...
For ease, the version of the data used in the analysis below is linked here:  [publishedweek302021.xlsx](uploadFiles/publishedweek302021.xlsx).

You will notice that again this is an Excel file, not a .csv file. To make compatible with the analysis in Spiegelhalter (2020), we need death counts for England and Wales, by age and sex. From the `Contents` worksheet we can see that these data can be found in the `Covid-19 - Weekly occurrences` worksheet as shown in Figure \@ref(fig:coviddeaths). 
...
* data for males in cells B33:CG53;
* data for females in cells B55:CG75.
...
```{r, eval = FALSE}
## read in the correct worksheet from the .xlsx
deaths <- read_excel("publishedweek302021.xlsx", 
        sheet = "Covid-19 - Weekly occurrences", range = "B33:CG53") %>%
    mutate(sex = "male") %>%
    rbind(
        read_excel("publishedweek302021.xlsx", 
            sheet = "Covid-19 - Weekly occurrences", range = "B55:CG75") %>%
        mutate(sex = "female")
    )
deaths
```
...
```{r, message = FALSE, echo = FALSE}
## read in the correct worksheet from the .xlsx
deaths <- read_excel("covid/uploadFiles/publishedweek302021.xlsx", 
        sheet = "Covid-19 - Weekly occurrences", range = "B33:CG53") %>%
    mutate(sex = "male") %>%
    rbind(
        read_excel("covid/uploadFiles/publishedweek302021.xlsx", 
            sheet = "Covid-19 - Weekly occurrences", range = "B55:CG75") %>%
        mutate(sex = "female")
    )
deaths
...
Notice that the first column relates to the age categories, and has been read in as a `character` column (although the column name is incorrect due to the way the data were stored---we will correct this shortly). The other columns are the death counts, and have been read in as `numeric` values, as expected. The column names are all nonsensical, due to the formatting of the original file, so let's sort those out now. Let's read in the row of dates (cells C6:CG6) from the Excel file, which should return an empty `tibble` object with column names corresponding to the cells read in[^cells]. We then pipe this to the `colnames()` function, which should return a vector of column names:

[^cells]: since `col_names = TRUE` by default in the `read_excel()` function

```{r, eval = FALSE}
## create vector of column names
dates <- read_excel("publishedweek302021.xlsx", 
        sheet = "Covid-19 - Weekly occurrences", range = "C6:CG6") %>%
    colnames()
dates
```
...
```{r, message = FALSE, echo = FALSE}
## create vector of column names
dates <- read_excel("covid/uploadFiles/publishedweek302021.xlsx", 
        sheet = "Covid-19 - Weekly occurrences", range = "C6:CG6") %>%
    colnames()
dates
```
...
```{info, title = "Note", collapsible = FALSE}
There are lots of useful things you can do with date objects. Although we are not using it here, the `lubridate` package, which is part of the `tidyverse` has lots of really useful features. If you're interested, check out the webpage (and asociated cheat sheet) [here](https://lubridate.tidyverse.org/).
```
...
Group the `age` column in `pop_sub` to match the groups above and then aggregate together the population sizes appropriately by age group. [**Hint**: it might be easier to create a new `age_grp` column to contain the grouping information, and then use `mutate()` with `ifelse()` statements to convert numeric ages to groups. You can replace the `age` column with `age_grp` at the end.]
```
...
``{r, eval = FALSE}
## extract relevant data
pop_sub <- mutate(pop_sub, age_grp = ifelse(age < 1, "<1", NA)) %>%
    mutate(age_grp = ifelse(age >= 1 & age <= 4, "1-4", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 5 & age <= 9, "5-9", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 10 & age <= 14, "10-14", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 15 & age <= 19, "15-19", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 20 & age <= 24, "20-24", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 25 & age <= 29, "25-29", age_grp))%>%
    mutate(age_grp = ifelse(age >= 30 & age <= 34, "30-34", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 35 & age <= 39, "35-39", age_grp))%>%
    mutate(age_grp = ifelse(age >= 40 & age <= 44, "40-44", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 45 & age <= 49, "45-49", age_grp))%>%
    mutate(age_grp = ifelse(age >= 50 & age <= 54, "50-54", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 55 & age <= 59, "55-59", age_grp))%>%
    mutate(age_grp = ifelse(age >= 60 & age <= 64, "60-64", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 65 & age <= 69, "65-69", age_grp))%>%
    mutate(age_grp = ifelse(age >= 70 & age <= 74, "70-74", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 75 & age <= 79, "75-79", age_grp))%>%
    mutate(age_grp = ifelse(age >= 80 & age <= 84, "80-84", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 85 & age <= 89, "85-89", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 90, "90+", age_grp)) %>%
...
Since `age` is now a `character` type, R will plot or summarise in [**lexicographical**](https://en.wikipedia.org/wiki/Lexicographic_order) ordering. This means that e.g. the `10-14` group comes ***before*** the `5-9` group (since `1` comes before `5`). One way to overcome this is to convert `age` to a `factor`, and explicitly set the `levels` of the factor to match the ordering that you want. Since we will join to another dataset later on, we will do this temporarily when we need it, rather than converting the `age` column now. Hence we will create a helper vector of levels as below, which we can call when needed.

```{r}
## set age level orders
age_levels <- c("<1", "1-4", "5-9", "10-14", "15-19",
...
```{task}
Produce a plot of the log-population fatality rates from COVID-19 against age, stratified by sex. Use a temporary `mutate()` call to convert the `age` column into a `factor` and then pipe into `ggplot()`. [See the solution for some neat little tricks to make the $x$-axis labels more readable, the $y$-axis labels match Figure @ref(fig:spiegelhalter) and tidy up the legend heading.]
```

```{solution}
    
``{r}
mutate(PFR, age = factor(age, levels = age_levels)) %>%
    ggplot() +
...
PFR <- select(lifetables, age, sex, qx) %>%
    mutate(age_grp = ifelse(age < 1, "<1", NA)) %>%
    mutate(age_grp = ifelse(age >= 1 & age <= 4, "1-4", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 5 & age <= 9, "5-9", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 10 & age <= 14, "10-14", age_grp)) %>%
    mutate(age_grp = ifelse(age >= 15 & age <= 19, "15-19", age_grp)) %>%
