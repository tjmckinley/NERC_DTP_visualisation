...
R has very powerful and flexible plotting capabilities, that enable you to produce almost any type of plot (standard or customised) that you like. The downside is that the syntax can be verbose, so for complex plots it can be time consuming to code up. 
...
We will not focus on using R's base graphics capabilities here, but will provide some comparison code for some of the plots (if you are interested) in order to show you how you can generate similar plots with the base graphics capabilities.
```
...
```{task}
The file [`ff.rds`](advis/uploadFiles/ff.rds) contains the data from this experiment. Download this data set to your working directory, read it into R using the command:
...
plot(ff$thorax[ff$partner.type == "Control"], 
    ff$longevity[ff$partner.type == "Control"], pch = 20,
    xlab = "Thorax length (mm)", 
    ylab = "Longevity (days)", 
    main = "Controls",
...
## fit linear model and add fitted line
## using abline() - you could also do this
## using lines() and a bunch of coordinates
temp_lm <- lm(longevity ~ thorax, data = ff[ff$partner.type == "Control", ])
abline(coef(temp_lm)[1], coef(temp_lm)[2])

titles <- c("One partner", "Eight partners")
partners <- c("1", "8")
for(i in 1:length(titles)) {
    ## extract subset of data for brevity in
    ## subsequent code
    ...
    ## fit linear model and add fitted line
    ## using abline()
    temp_lm <- lm(longevity ~ thorax, data = temp[temp$partner.type == "Virgin", ])
    abline(coef(temp_lm)[1], coef(temp_lm)[2])
    temp_lm <- lm(longevity ~ thorax, data = temp[temp$partner.type == "Inseminated", ])
    abline(coef(temp_lm)[1], coef(temp_lm)[2], lty = 2)
    
    ## add legend
    legend(0.65, 95, pch = c(20, 1), lty = 1:2, legend = c("Virgin", "Inseminated"))
}
## reset par
par(mfrow = c(1, 1))
``
