...
```{footnotes}
or `tibble` objects---see next session...
```
...
Now have a go at some of the exercises in Section 5.3 of the lecture notes. This uses the `iris` data set in R to illustrate some of these concepts, but this time generating a scatterplot.
...
Have a go at Section 5.3.6 of the workshop.
...
Add some smoothed lines to your `iris` scatterplot. See Section 5.3.7 of the notes.
...
Work through the tasks in Section 5.4 of the notes, and don't forget to ask if you don't understand anything.
...
We have just begun to scratch the surface with what `ggplot2` can do (Section 5.7 will introduce ways to make animations and interactive plots using the `gganimate` and `plotly` packages).
...
In the next practical we will see how we can use `tidyverse` to facilitate complex **data wrangling** tasks in R.
